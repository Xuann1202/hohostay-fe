...
// // 一次載入所有飯店並處理
// const loadAllHotelsAndProcess = async () => {
//     try {
//       const allHotels = await hotelAPI.getHotelsByOwner({ page: 1, size: 1000 })
//       const hotelList = allHotels.content || []
//       //儲存完整列表
//       allHotelList.value = hotelList
//       //分頁顯示
//       applyPagination()

//       //提取城市和飯店類型
//       extractCitiesAndHotelTypes(hotelList)
//       extractHotelTypesFromHotels(hotelList)
  
//     } catch (error) {
//       console.error('載入所有飯店失敗：', error)
//     }
//   }
...
const fetchHotels = async (page = currentPage.value) => {
  try {
    ...
    // 註解：不再需要從飯店中提取飯店類型
    // 因為 loadHotelTypes() 已經在 onMounted 中載入了所有飯店類型
...
// 載入所有城市和行政區（直接從 API 獲取，不再從飯店列表中提取）
const loadCitiesAndDistricts = async () => {
  try {
    loadingCities.value = true
    
    // 直接從 API 獲取所有行政區（包含城市信息）
    // 這樣做的好處：
    // 1. 不需要請求 size=1000 的飯店列表，效能更好
    // 2. 即使房東沒有飯店，也能顯示所有城市選項
    // 3. 邏輯更簡單，不需要處理從飯店提取的複雜邏輯
    let cityMap = new Map()
    ...
    try {
      ...
      // 如果 API 失敗，使用空列表
      if (import.meta.env.DEV) {
        console.warn('無法從 API 獲取城市和行政區信息:', error)
      }
    }

    // 將 Map 轉換為數組並按照中文名稱排序
    cities.value = Array.from(cityMap.values()).sort((a, b) => 
...
// 從 API 載入所有飯店類型（直接使用 API，不再從飯店列表中提取）
const loadHotelTypes = async () => {
  try {
    // 直接使用 API 獲取所有飯店類型
    // 這樣做的好處：
    // 1. 不需要請求 size=1000 的飯店列表，效能更好
    // 2. 即使房東沒有飯店，也能顯示所有飯店類型選項
    // 3. 邏輯更簡單，不需要處理從飯店提取的複雜邏輯
    ...
    // 如果 API 失敗，記錄錯誤並使用空列表
    console.error('載入飯店類型失敗：', error)
    availableHotelTypes.value = []
  }
}

// 註解：此函數已不再使用
// 因為 loadHotelTypes() 已經直接從 API 載入所有飯店類型
// 不再需要從飯店列表中提取，避免重複請求 size=1000 的資料
// 
